...
@Component({
  selector: 'app-balance-trimestral-graph',
  templateUrl: './balance-trimestral-graph.component.html',
  styleUrls: ['./balance-trimestral-graph.component.scss'],
})
export class BalanceTrimestralGraphComponent implements OnInit {
  ...
  async load() {
    ...
    this.financyService
      .getTrimestralBalance(this.filter)
      .then((response) => {
  ...
  graphGenerator() {
    if (this.resultDataGrath) {
      ...
      if (!this.canvasGraph) {
        this.canvasGraph = document.getElementById(
          'balanceTrimestralCanvasGraph'
        );
      }
